document.addEventListener('DOMContentLoaded', function () {
    ...
    function showMessage(text, type = 'info') {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                if (messageElement.parentNode === document.body) { // Check if still in DOM
    ...
    function processLocatorPairs(pairs, prefix = "") {
        ...
        // Find available locator pairs
        const idPair = pairs.find(p => p.type === 'id');
        const xpathPair = pairs.find(p => p.type === 'xpath');
        const namePair = pairs.find(p => p.type === 'name'); // Find name pair
        const cssPair = pairs.find(p => p.type === 'cssSelector');

        let chosenActivePair = null;
        // Priority:
        // 1. Any non-empty XPath (generated by generateIndexedXPath in content.js)
        // 2. ID (if non-empty)
        // 3. Name (if non-empty)
        // 4. CSS (if non-empty)
        // Fallbacks (if all above have empty expressions or don't exist):
        //    XPath -> ID -> CSS -> Name (using the first available pair, even if expression is empty)

        if (xpathPair && xpathPair.expression) { // Prioritize any non-empty XPath
            chosenActivePair = xpathPair;
        } else if (idPair && idPair.expression) {
            chosenActivePair = idPair;
        } else if (namePair && namePair.expression) {
            chosenActivePair = namePair;
        } else if (cssPair && cssPair.expression) {
            chosenActivePair = cssPair;
        } else {
            // Fallback: If all preferred locators have empty expressions or are missing,
            // use the first available pair type in order: XPath, ID, CSS, Name.
            if (xpathPair) {
                chosenActivePair = xpathPair;
            } else if (idPair) {
                chosenActivePair = idPair;
                ...
                chosenActivePair = namePair;
                 ...
                 // If absolutely no pairs were found or had expressions, default to empty XPath
                 // This path should ideally not be hit if the initial `!pairs || pairs.length === 0` check is effective
                 // and `assignLocatorsToAttributes` always provides known types.
                 activeLine = `        ${attributePrefix}ocatorType="xpath" ${attributePrefix}ocatorExpression=""\n`;
                 return { activeLine, commentedLines }; // Return early
            }
        }

        if (chosenActivePair) {
            // Ensure expression is an empty string if null/undefined, especially from fallback.
            const expr = (chosenActivePair.expression !== null && chosenActivePair.expression !== undefined) ? chosenActivePair.expression : "";
            activeLine = `        ${attributePrefix}ocatorType="${chosenActivePair.type}" ${attributePrefix}ocatorExpression="${expr}"\n`;
        } else {
            // This case is hit if `pairs` is not empty, but no chosenActivePair was set (e.g., unknown locator types).
            activeLine = `        ${attributePrefix}ocatorType="xpath" ${attributePrefix}ocatorExpression=""\n`;
         }
        ...
        if (namePair && namePair.expression && (!chosenActivePair || chosenActivePair.type !== 'name')) { // Comment for name
            commentedLines.push(`    <!-- ${attributePrefix}ocatorType="name" ${attributePrefix}ocatorExpression="${namePair.expression}" -->\n`);
        }
    ...
    function assignLocatorsToAttributes(attributes, locators, prefix = "") {
        ...
        // Priority (New): Name
        if (locators.name && String(locators.name).trim() !== '') {
            currentLocatorPairs.push({ type: 'name', expression: String(locators.name) });
        }
    ...
    function convertEventsToXml(events) {
        ...
        events.forEach(event => {
            ...
            switch (event.type) {
                ...
                case 'selectChange':
                    tagName = 'SelectElement'; // As per request
                    const selectLocs = e.locators || {};
                    eventAttributes = {
                        input: escapeXml(e.text || ''), // 'input' attribute from selected option's text
                        timestamp: relativeTimestamp
                    };
                    assignLocatorsToAttributes(eventAttributes, selectLocs); // Populates eventAttributes.locatorPairs
                    if (e.iframeSrc) eventAttributes.iframe = escapeXml(e.iframeSrc);
                    break;
    ...
    function escapeXml(string) {
        ...
        // Now includes escaping single quotes (') to &apos;
